...
def analizar_codigo():
    ...
    for tok in vocabulario.lexer:
        ...
        result_tree.insert("", 'end', values=(tok.type, tok.value, pos, "X" if tok.type == 'IDENTIFICADOR' else "", "X" if tok.type == 'NUMERO' else "", "X" if tok.type == 'OPERADOR' else "", "X" if tok.type == 'SIMBOLO' else ""))
...
boton_analizar = tk.Button(frame_izquierda, text="Analizar", command=analizar_codigo)
...
result_tree = ttk.Treeview(frame_derecha, columns=("Valor", "Token", "Posición", "IDENTIFICADOR", "OPERADOR", "NUMERO", "SÍMBOLO"), show="headings")
